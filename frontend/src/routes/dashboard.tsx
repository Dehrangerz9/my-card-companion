...
import { CreditCard, LogOut, ChevronRight, Wifi, ShieldOff } from "lucide-react";
import { checkAuth, logout } from "../lib/auth-store";
import { mockCards, formatBRL } from "../lib/mock-data";
import { useBlockedCards } from "../lib/card-store";
                ...
                <div className={`${card.gradient} relative aspect-[1.6/1] overflow-hidden rounded-2xl p-5 transition-transform group-hover:scale-[1.02]`}>
                  {isBlocked(card.id) && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 rounded-2xl bg-black/60 backdrop-blur-sm">
                      ...
                      <span className="text-sm font-bold uppercase tracking-widest text-white">Bloqueado</span>
                    </div>
                  )}
